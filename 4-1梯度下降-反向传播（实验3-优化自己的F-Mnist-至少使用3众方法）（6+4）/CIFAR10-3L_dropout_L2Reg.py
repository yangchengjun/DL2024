...
'''
基于一般经验，MLP模型在CIFAR-10数据集上的最高准确率通常在50%到60%之间，并且即使使用正则化、数据增强等技巧，也很难突破这个范围。
...
L2:正则化
 Average Loss: 0.8956, Training Accuracy: 68.15%
Test Accuracy: 53.79%, Test Loss: 1.4243, Epoch: 9

4L_dropout_L2Reg:
Average Loss: 1.5912, Training Accuracy: 44.06%
Test Accuracy: 47.89%, Test Loss: 1.4929, Epoch: 9


3L_dropout_L2Reg:
 Average Loss: 1.7140, Training Accuracy: 39.06%
Test Accuracy: 45.65%, Test Loss: 1.5510, Epoch: 9
'''
...
# 主函数
def main():
    print('3L_dropout_L2正则化（权重衰减）')
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    print(f'Using device: {device}')
    train_loader, test_loader = load_dataset()
    model = MLP().to(device)
    optimizer = optim.Adam(model.parameters(), lr=0.001,weight_decay=1e-4)  #0.0001,L2正则化（权重衰减）,在优化器中引入L2正则化，通过惩罚权重过大的参数，防止模型过拟合
    criterion = nn.CrossEntropyLoss()

    writer = SummaryWriter('runs/LOG_CIFAR10_3L_dropout_L2Reg')
    num_epochs = 10
